/*  Differential harness: pdgzip vs zlib.
    Invariants enforced:
...
int LLVMFuzzerTestOneInput(const uint8_t * data, size_t size) {
  ...
  uint8_t * p_out = NULL, * z_out = malloc(Z_CAP);
  size_t p_len = 0;  int p_rc = fz_decode(data, size, &p_out, &p_len, 0);
  if (!z_out)
    { free(p_out);  return 0; }
  z_stream zs = {0};
  ...
  zs.next_out  = z_out;
  zs.avail_out = Z_CAP;
  int z_ok = 0, z_bomb = 0;  size_t z_len = 0;
  ...
  } else if (p_ok && !z_ok)  __builtin_trap();
  else if (!p_ok && z_ok)    __builtin_trap();
  free(p_out);  free(z_out);  return 0;
}
