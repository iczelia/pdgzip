...
typedef struct { const uint8_t * data; size_t pos, len; } fz_src_t;
...
static int fz_decode(const uint8_t * data, size_t len,
  ...
  void * scratch = aligned_alloc(pdgzip_state_align(), pdgzip_state_size());
  if (!scratch) return -1;
  pdgzip_t * gz = pdgzip_init(scratch, &cfg);
  if (!gz)
    { free(scratch);  return -1; }
  size_t cap = 4096, used = 0;  uint8_t * buf = malloc(cap);
  if (!buf)
    { free(scratch);  return -1; }
  int rc = 0;
  for (;;) {
